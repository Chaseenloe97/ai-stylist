    ...
    <div className="min-h-screen bg-gradient-to-b from-cream-50 to-white pb-24">
      ...
      <div className="relative overflow-hidden bg-white border-b border-cream-200">
        <div className="max-w-4xl mx-auto px-6 py-16 text-center">
          <div className="w-20 h-20 mx-auto mb-6 bg-ink-900 flex items-center justify-center">
          ...
          <p className="text-lg text-ink-700 max-w-2xl mx-auto leading-relaxed font-light">
            {summary}
          </p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        {/* Core Aesthetic */}
        <div className="card-luxury p-10 mb-8">
          <h2 className="text-sm font-semibold tracking-widest uppercase text-ink-600 mb-6">
            Core Aesthetic
          </h2>
        ...
        <div className="card-luxury p-10 mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {topTags?.map((tag, idx) => (
              <div key={idx} className="border border-cream-200 p-6 text-center hover:border-gold-400 transition-colors duration-300">
                <div className="text-sm font-light text-ink-900 capitalize">
                  {tag.replace('-', ' ')}
                </div>
              </div>
            ))}
        ...
        <div className="card-luxury p-10 mb-8">
          ...
          >
            <h2 className="text-sm font-semibold tracking-widest uppercase text-ink-600">
              Curated Guidance
            </h2>
            <svg
              className={`h-5 w-5 transform transition-transform duration-300 text-ink-900 group-hover:text-gold-500 ${
          ...
          {showInsights && (
            <div className="mt-8 space-y-4 animate-fade-in">
              {insight.tips.map((tip, idx) => (
                <div key={idx} className="flex items-start space-x-4 py-4 border-t border-cream-200 first:border-t-0">
                  <div className="text-gold-500 text-sm font-serif mt-1">
                    {idx + 1}
                  </div>
                  <p className="flex-1 text-ink-700 leading-relaxed font-light">
                    {tip}
                  </p>
                </div>
              ))}
